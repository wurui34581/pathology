    ...
    switch (action.type) {
      ...
      case 'RELEASE':
        if (model.mode === 'DRAW') {
          ...
          let loc = annotations[annotations.length-2][1].d.replace('M','').split(' ')
          console.log(model)
          let root = document.getElementById('openseadragon1');
          ...
          let labelSelect = '<div style="color: #e6e6e6ba; padding-bottom: 5px; border-bottom: solid 1px #e6e6e600">请选择标注备注</div>' +
            ...
            '<option value="benign">良性病变</option>' +
            '<option value="dcis">原位癌</option>' +
            '<option value="nst">浸润性癌</option>' +
          ...
          wrapper.innerHTML = labelSelect;
          root.appendChild(wrapper)
          let lastChild = root.childNodes[2] && root.childNodes[2];
          lastChild && root.removeChild(lastChild)

          let changeLabel = document.getElementById('selectLabel')
          let submitLabel = document.getElementById('submit')
          let selectedValue
          changeLabel.onchange = (e)=>{
